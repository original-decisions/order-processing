...
begin
	...
	CREATE INDEX [IX_ZoneId] ON [order].[DeliveryCharges]([ZoneId])
	CREATE INDEX [IX_DeliveryMethodId] ON [order].[DeliveryCharges]([DeliveryMethodId])
end
...
IF  NOT EXISTS (SELECT * FROM sys.objects 
	WHERE object_id = OBJECT_ID(N'[order].[DeliveryZones]') AND type in (N'U'))
begin
CREATE TABLE [order].[DeliveryZones] (
    [Id] [int] NOT NULL IDENTITY,
    [Name] [nvarchar](50) NOT NULL,
    [Description] [nvarchar](1000) NOT NULL,
    [Code] [nvarchar](128) NOT NULL,
    ...
    [DateUpdated] [datetime] NOT NULL,
    [DateCreated] [datetime] NOT NULL,
    CONSTRAINT [PK_order.DeliveryZones] PRIMARY KEY ([Id])
)
...
CREATE TABLE [order].[OrderTypes] (
    ...
    CONSTRAINT [PK_order.OrderOrderTypes] PRIMARY KEY ([OrderId], [OrderTypeId])
)
...
if not exists (SELECT  name
                FROM    sys.foreign_keys
                WHERE   name = 'FK_order.DeliveryCharges_order.DeliveryMethods_DeliveryMethodId')
		begin
		ALTER TABLE [order].[DeliveryCharges] ADD CONSTRAINT [FK_order.DeliveryCharges_order.DeliveryMethods_DeliveryMethodId] FOREIGN KEY ([DeliveryMethodId]) REFERENCES [order].[DeliveryMethods] ([Id])
end

if not exists (SELECT  name
                FROM    sys.foreign_keys
                WHERE   name = 'FK_order.DeliveryCharges_order.DeliveryZones_ZoneId')
		begin
		ALTER TABLE [order].[DeliveryCharges] ADD CONSTRAINT [FK_order.DeliveryCharges_order.DeliveryZones_ZoneId] FOREIGN KEY ([ZoneId]) REFERENCES [order].[DeliveryZones] ([Id])
		end
